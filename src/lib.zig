...
pub fn Lexer(comptime ReaderType: type) type {
    return struct {
        ...
        allocator: Allocator,
        reader: std.io.PeekStream(.{ .Static = 1 }, std.io.BufferedReader(max_token_len, ReaderType)),
        ...
        fn init(allocator: Allocator, reader: ReaderType) !Self {
            var res = Self{
                .allocator = allocator,
                .reader = std.io.peekStream(1, std.io.bufferedReaderSize(max_token_len, reader)),
                .token_buf = undefined,
...
test "single simple tag" {
    ...
    try testing.expectEqual(Token{ .tag_open = 1 }, (try lex.nextToken()).?);
    try testing.expectEqualStrings(usfm[4..], (try lex.nextToken()).?.text);
    try testing.expectEqual(@as(?Token, null), try lex.nextToken());
}
...
test "two simple tags" {
    ...
    try testing.expectEqual(Token{ .tag_open = 1 }, (try lex.nextToken()).?);
    try testing.expectEqualStrings(usfm[4..30], (try lex.nextToken()).?.text);
    try testing.expectEqual(Token{ .tag_open = 2 }, (try lex.nextToken()).?);
    try testing.expectEqualStrings("3.0", (try lex.nextToken()).?.text);
    try testing.expectEqual(@as(?Token, null), try lex.nextToken());
}
...
test "single attribute tag" {
    ...
    try testing.expectEqual(Token{ .tag_open = 1 }, (try lex.nextToken()).?);
    try testing.expectEqualStrings("hello ", (try lex.nextToken()).?.text);
    try testing.expectEqual(Token.attribute_start, (try lex.nextToken()).?);
    ...
    try testing.expectEqual(Token{ .tag_close = 1 }, (try lex.nextToken()).?);
    try testing.expectEqual(@as(?Token, null), try lex.nextToken());
}
...
test "self closing tag" {
    ...
    try testing.expectEqual(Token{ .tag_open = 1 }, (try lex.nextToken()).?);
    try testing.expectEqualStrings("hello", (try lex.nextToken()).?.text);
    try testing.expectEqual(Token{ .tag_close = 0 }, (try lex.nextToken()).?);
    try testing.expectEqual(@as(?Token, null), try lex.nextToken());
}
...
test "full line" {
    ...
    try testing.expectEqual(Token{ .tag_open = 1 }, (try lex.nextToken()).?);
    try testing.expectEqualStrings("1 ", (try lex.nextToken()).?.text);
    try testing.expectEqual(Token{ .tag_open = 2 }, (try lex.nextToken()).?);
    try testing.expectEqual(Token.attribute_start, (try lex.nextToken()).?);
    const attribute = (try lex.nextToken()).?.attribute;
    ...
    try testing.expectEqual(Token{ .tag_open = 3 }, (try lex.nextToken()).?);
    try testing.expectEqualStrings("In", (try lex.nextToken()).?.text);
    try testing.expectEqual(Token{ .tag_close = 3 }, (try lex.nextToken()).?);
...
test "full file" {
    ...
    while (tok != null) tok = try lex.nextToken();
}
